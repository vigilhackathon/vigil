...
| PR6 | VIG-11 | Patient text channel (MOCK web thread) + enroll UI | 🟡 in review | Claude (guardrail clone) | `pr06-mock-thread` | **PR #9 open.** Click-through verified: DOB gate → phone capture → baseline walk → red-chip escalation (alert fired) → live-model bank turn. Adds /api/state + /api/checkin + /api/confirm |
